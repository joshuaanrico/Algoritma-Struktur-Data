...
/* Diperkenankan pula jika ingin menggunakan Fungsi */
void HitungResource (List L, float n) {
/* List tidak mungkin kosong */
/* Jangan sampai pengambilan material melebihi n */
    address P, point, temp;
    float total = 0;
    int x = 0;
    P = First(L);
    while ( total !=n && P!=point){
        total = total + Info(P);
        //cout<<total<<endl;
        if(total >n){
            total = total - Info(P);
